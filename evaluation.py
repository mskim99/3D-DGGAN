...
for i in range (0, 18):
    ...
    fake_B = np.expand_dims(fake_B, 0)
    real_B = np.expand_dims(real_B, 0)
    ...
    # for th in [0.19, 0.2175, 0.245, 0.2725, 0.3, .45, .475, .5, .525, .55]:
        ...
        dice = ef.dice_factor2(fake_B_tensor, real_B_tensor, th)
    ...
    # IoU per taxonomy
    test_iou = {'n_samples': 0, 'iou': [], 'dice': [], 'accuracy': [], 'precision': [], 'recall': [], 'f1-score': []}
    test_iou['n_samples'] += 1
    test_iou['iou'].append(sample_iou)
    test_iou['dice'].append(sample_dice)
...
print('[INFO] Average : L1Loss = %.6f, Dice Scores = %.6f, PSNR score =  %.6f, SSIM score =  %.6f, UQI score =  %.6f, '
      'MS-SSIM score =  %.6f, IW-SSIM score =  %.6f, GMSD score =  %.6f, MS_GMSD score =  %.6f, VSI score =  %.6f, HaarPSI score =  %.6f, MDSI score =  %.6f, ERGAS score =  %.6f, SCC score =  %.6f, '
      'SAM score =  %.6f, VIF score =  %.6f, R2 score =  %.6f, TV score =  %.6f, brisque score =  %.6f'
      % (L1_losses.avg, dice_scores.avg, V_PSNRs.avg, V_SSIMs.avg, V_uqis.avg, V_msssims.avg, V_iwssims.avg, V_gmsds.avg, V_ms_gmsds.avg, V_vsis.avg, V_haarpsis.avg, V_mdsis.avg, V_ergass.avg, V_sccs.avg
         , V_sams.avg, V_vifps.avg, V_R2s.avg, V_TVs.avg, V_brisques.avg))
eval_factors = ['iou', 'dice', 'accuracy', 'precision', 'recall', 'f1-score']
print('============================ RESULTS ============================')
print('#Sample', end='\t')
for th in [.25, .2875, .325, .3625, .4, .45, .475, .5, .525, .55]:
# for th in [0.19, 0.2175, 0.245, 0.2725, 0.3, .45, .475, .5, .525, .55]:
